module Converter
    ...
    class ComputeStim
        ...
        def generate_stim netlist=nil, ht="og_s38417", save_explicit: "explicit_stim.txt", freq: 1.1, compute_all_transitions: false, all_outputs: false, all_insert_points: false
            ...
            @insert_points.each do |insert_point|
                ...
                # if insert_point.get_full_name == "And25880_i0" #!DEBUG
                #     pp 'here'
                # end
        ...
        def backpropagate2 event
            ...
            while !@events_to_process.empty? and @events_to_process != [nil]
                ...
                if !(g.instance_of? Netlist::Port and g.is_global? and g.is_input?)
                    ...
                    if !g.instance_of?(Netlist::Port) and !g.instance_of?(Netlist::Not) and !g.instance_of?(Netlist::Buffer)
                        if g.tag == :target_path and g.get_sink_gates.all?{|sink| sink.tag == :secondary_path} and g.get_sink_gates.none?{|sink| sink.tag == :target_path}
                            g.tag = :secondary_path
                        end

                        input_tag = [g.get_source_gates[0].tag, g.get_source_gates[1].tag]
                        # Si les entrées sont toutes deux sur un target_path
                        if input_tag[0] == :target_path and input_tag[1] == :target_path
                            # Fixer l'entrée i0 comme un secondary path 
                            g.get_source_gates[0].tag = :secondary_path
                            # Appeler compute_transitions(e)
                            e_inputs = compute_transitions(e)
                            # Si le résultat est nil 
                            if e_inputs == nil or e_inputs == [nil]
                                # Fixer i0 comme target_path et i1 comme le secondary path
                                g.get_source_gates[0].tag = :target_path
                                g.get_source_gates[1].tag = :secondary_path
                                # Reinitialiser e.possible
                                e.forbidden = []
                                e.possible = []
                                # Appeler compute_transitions(e)
                                e_inputs = compute_transitions(e)
                        ...
                        # Sinon si i0 est un secondary path
                        elsif input_tag[0] == :secondary_path and input_tag[1] == :target_path and e.possible.empty?
                            # Fixer i0 comme target_path et i1 comme le secondary path
                            g.get_source_gates[0].tag = :target_path
                            g.get_source_gates[1].tag = :secondary_path
                            # Supprimer les interdits pour la configuration précédentes (i0 comme target_path)
                            e.forbidden = []
                            # Réinitialiser e.possible
                            e.possible = []
                            # Appeler compute_transitions(e)
                            e_inputs = compute_transitions(e)
                        # Sinon 
                    ...
                    if e_inputs.nil? or e_inputs == [nil] or e_inputs == :conflict
                        ...
                        # ? Return a keyword (symbol) in compute_transitions indicating an event on the same signal is causing an absence of solution
                            ...
                            if events_with_possibilities.empty? or e_inputs != :conflict
                                ...
                                e.forbidden = []
                                @events_to_process << e
                                backtrack(events_with_possibilities[0].children.events[0])
                            end
                    else
                        ...
                        if e_inputs.all?{|x| x.signal.tag == :target_path} or e_inputs.none?{|x| x.signal.tag == :target_path}
                            ...
                            # e_inputs.sort_by{|e| e.signal.cumulated_propag_time}.each do |e|
                            #     if @signal_events[e.signal].none?{|x| x.match? e}
                            #         @events_to_process << e
                            #     end
                            # end
                        ...
                        #                 e.is_non_stable_transition? and e.signal.get_sinks.include? insert_point
                        #             end
                        #         else
                        #             e.is_non_stable_transition? and e.signal.get_output.get_sinks.include?insert_point
                        #         end
                        #     } and @insert_point_observable.nil?
                ...
                if @events_to_process.empty? or @events_to_process == [nil]  and !get_inputs_events.empty? # * Should break the while, before check inputs
                    ...
                    # if (@forbidden_vectors.intersect? test_vec_couple)
                    #     backtrack(e)
                    # end
    ...
    # ! : The following constraints are necessary for inertial integer delay model  
            ...
            #         if previous_event.afterward_value != event.previous_value
            #             return false
            #         end
            #     end
            #     if !next_event.nil?
            #         if event.afterward_value != next_event.previous_value
            #             return false
            #         end
            #     end
            # end
        ...
        def compute_transitions event, input_transition = nil
            ...
            possible_inputs_events.select! do |proposed_events|
                ...
                if new_input_events.empty?
                    true
                    # if is_convertible?(get_inputs_events)
                        # return proposed_events
                    # end
                else
                    is_convertible?(get_inputs_events + new_input_events)
                    # if is_convertible?(get_inputs_events + new_input_events)
                        # return proposed_events
                    # end 
                end
            end
        ...
        # def replace_X_values v, fv # vector, forbidden_vector
        #     v_diff = (v - fv)
        #     if v_diff.all?{|e| e == "X"}
        #         fv.each_with_index do |e, i|
        #             if v[i] == "X"
        #                 if e == "0"
        #                     v[i] = "1"
        #                 else
        ...
        #         end
        #     elsif v_diff.empty?
        #         return nil # Forbidden vector encountered
        #     else 
        #         # Replaces X with a default value
        #         v = v.join.tr("X","1").chars # ! Default value is "1"
        #     end

        #     return v
        # end

        # def avoid_forbidden_vector(test_vec_couple)
        ...
        #         fv = fv.chars
        #         ov = replace_X_values(ov, fv)
        #         av = replace_X_values(av, fv)

        #         if ov.nil? or av.nil? # forbidden vector encountered 
        #             return nil
        #         end
        #     end

        #     return ov.join, av.join
        # end
