...
module Converter
  class GenCompTestbench
    ...
    def gen_init_portmap
      ...
      @netlist_init_data[:ports][:in].length.times do |pname|
        @portmap_init.concat "       tb_in(#{pname})"
        @portmap_init.concat ", \n"
      end
      @netlist_init_data[:ports][:out].length.times do |pname|
    ...
    def gen_alt_portmap
      ...
      @portmap_alt = ''
      # @portmap.concat "       nom_clk,\n"
      @netlist_alt_data[:ports][:in].length.times do |pname|
        @portmap_alt.concat "       tb_in(#{pname})"
        @portmap_alt.concat ", \n"
      end
      @netlist_alt_data[:ports][:out].length.times do |pname|
  ...
  class GenRealflowTestbench
    ...
    def extract_stim_file_info(stim_file)
      raise "File '#{stim_file}' foes not exist." unless File.exist?(stim_file)

      first_line = `head -n 1 #{stim_file}`
      header = first_line.chomp.split(';')
      raise 'Error: Only stim files generated by Netenos are handled.' unless header[0] == '# Stimuli sequence'
