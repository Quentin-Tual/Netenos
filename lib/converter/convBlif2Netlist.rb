  ...
  class ConvBlif2Netlist # ! Legacy / associated to Hyle parser, not used and not maintained
    ...
    def gen_genlib
      ...
      Netlist.get_gtech.each do |klass|
        ...
        unless %w[Buffer Not].include? class_shortname
    ...
    def convert(path, truth_table_format: true)
      file_path = if truth_table_format
                    truth_table_2_gates(path)
                  else
                    path
                  end
      parse(file_path)
    ...
    def parse_outputs_line(line)
      ...
      output_names.each_with_index do |name, n|
        ...
        @sym_tab[:p_out][name] <= @sym_tab[:p_in][name] if @sym_tab[:p_in].key?(name)
      end
    ...
    def instantiateGate(gate_type)
      ...
      when 'BUF1'
        Netlist::Buffer.new
      when 'ZERO0'
        Netlist::Zero.new # ! Prendre en charge les constantes dans le circuit
      when 'ONE0'
        Netlist::One.new # ! Idem
      else
        type = gate_type.downcase[...-1].to_sym
        nb_inputs = gate_type[-1].to_i
        Netlist.create_gate(type, nb_inputs)
