            ...
            while !@events_to_process.empty? and @events_to_process != [nil]
                ...
                if !(g.instance_of? Netlist::Port and g.is_global? and g.is_input?)
                    ...
                    if e_inputs.nil? or e_inputs == [nil]
                        ...
                        # ! Si d'autres évènements sont déjà fixés sur g, vérifier qu'il n'existe plus d'autre possibilité pour lui. Si il en existe d'autre les tester (backtrack l'autre évènement et rajouter l'évènement courant dans la file à traiter) sinon backtrack e et next
                        backtrack(e)
                    else
                        ...
                        if e_inputs.any?{|e| e.signal.tag == :target_path}
                            e_inputs.sort_by{|e| e.signal.tag.to_s}.each do |e|
                            ...
                            e_inputs.sort_by{|e| e.timestamp}.each do |e|
                                if @signal_events[e.signal].none?{|x| x.match? e}
                                    @events_to_process << e
                                end
                            end
                ...
                if @events_to_process.empty? or @events_to_process == [nil] # * Should break the while, before check inputs
                    ...
                    # if test_vec_couple.include?('%0*b' % [8, 42])# or test_vec_couple.include?(('%0*b' % [8, 42]).reverse)
                    #     pp 'here'
                    # end
