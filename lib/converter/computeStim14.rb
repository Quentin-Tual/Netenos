module Converter
    ...
    class ComputeStim
        attr_accessor :decisions, :transitions, :test, :insert_point, :side_inputs, :stim_vec, :events_computed, :unobservables, :insert_points, :observables, :events_to_process, :control_path, :signal_events

        def initialize netlist, delay_model, forbidden_vectors = [], logging = false
            @netlist = netlist
            ...
            @logging = logging
            @logger = Logger.new("test.log") if logging
            # @test = 0
        end

        def analyse_netlist ht, delay_required = nil
            case ht
            when "og_s38417"
            ...
            when "buffer"
                if delay_required.nil? 
                    raise "Error : #{ht} HT type requires delay, not passed." 
                end
            else
        ...
        def generate_stim netlist=nil, ht="og_s38417", save_explicit: "explicit_stim.txt", freq: 1.1, compute_all_transitions: false, all_outputs: false, all_insert_points: false, ht_delay: nil
            @logger.info("Generating stimulus for #{ht} hardware trojan") if @logging
            ...
            @insert_points.each do |insert_point|
                ...
                if insert_point.get_full_name == "Nor21660_i0" #!DEBUG
                    pp 'here'
                end
                    ...
                    control_paths.each do |control_path|
                        ...
                        # downstream_outputs.each do |targeted_output|
                            ...
                            transitions_to_try.each do |transition|
                                ...
                                if res == :success
                                    puts "     |--[-] Solution found for control path #{control_path.collect{|e| e.get_full_name}}." if $VERBOSE
                                    @events_computed[insert_point][targeted_transition] = tmp
                                    break if !compute_all_transitions
                                    # end
                                else 
                                    puts "     |--[+] No solution satisfying constraintsfor control path #{control_path.collect{|e| e.get_full_name}}." if $VERBOSE
                                    tmp = nil
                                    res = nil
                                end
                            end
        ...
        def compute event, insertion_point
            @logger.info("Computing event (#{event.signal.get_full_name},#{event.timestamp},#{event.value}) on #{insertion_point.get_full_name}") if @logging
        ...
        def branch_possibilities_left? event
            # if event.possible.empty?
                if !event.children.nil?
                    return event.children.events.collect{|e| branch_possibilities_left?(e)}.find{|x| !x.nil?}
                else
                    if !event.parent.possible.empty?
                        return nil
                    else
                        return event.parent
                    end
        ...
        def backtrack wrong_event
            @logger.info("Backtracking event (#{wrong_event.signal.name},#{wrong_event.timestamp},#{wrong_event.value})") if @logging
            ...
            if wrong_event.signal.instance_of?(Netlist::Port) and wrong_event.signal.is_global? and wrong_event.signal.is_output?
                ...
                if !wrong_event.parent.signal.instance_of?(Netlist::Port) and !wrong_event.parent.signal.instance_of?(Netlist::Not) and !wrong_event.parent.signal.instance_of?(Netlist::Buffer)
                    # TODO : Vérifier si toutes les possibilités ont été explorées en aval du parent, si oui continuer, sinon backtrack une décision en bout de branche
                    possibility_to_try = branch_possibilities_left?((wrong_decision.events - [wrong_event])[0])
                    if !possibility_to_try.nil?
                        @events_to_process << wrong_event
                        wrong_event.forbidden = []
                        wrong_event.possible = []
                        wrong_event = possibility_to_try
                        wrong_decision = wrong_event.parent.children
                    # else
                    # # TODO : Vérifier si les autres évènements placés sur ce signal à d'autres timestamp ont d'autres possibilités, si oui tous les backtrack, sinon continuer 
                    end
                end
        ...
        def backpropagate2 event
            ...
            while !@events_to_process.empty? and @events_to_process != [nil]
                ...
                @logger.info("Backpropagating event (#{e.signal.name},#{e.timestamp},#{e.value})") if @logging

                g = e.signal

                # if g.name == "o10"# and e.timestamp == 9 and e.value == :S0 #!DEBUG
                #     pp 'here'
                # end
                ...
                if !(g.instance_of? Netlist::Port and g.is_global? and g.is_input?) 
                    ...
                    if e_inputs.nil? or e_inputs == [nil] or e_inputs == :conflict
                        # ! Infinity loop caused by this part
                        ...
                        e_inputs.each do |x| 
                            if x.signal.instance_of?(Netlist::Circuit) 
                                raise "Error : Event #{x.get_printable} abnormal."
                            end
                        end
                        ...
                        @logger.info("Decided of #{new_decision.get_printable_events}") if @logging
                        e_inputs.each do |x| 
                            if @signal_events[x.signal].none?{|y| y.match? x}
                ...
                if @events_to_process.empty? or @events_to_process == [nil]  and !get_inputs_events.empty? and !e.forbidden.any?{|d| d.include?(e)}# * Should break the while, before check inputs
                    ...
                    if test_vec_couple.nil? # forbidden_vector encountered
                        @logger.info("Forbidden vector computed, backtrack required") if @logging
                        
                        

                        # TODO : Limiter les évènements pouvant être backtrack à ceux concernant une entrée du signal de 'event'
                        if g.instance_of?(Netlist::Port)
                            inputs_events_changeable = get_inputs_events.select{|x| e.signal == x.signal}
                            # TODO : Limiter les évènements pouvant être backtrack à ceux dans la même zone temporelle que 'event' (avant ou après le ref_time)
                            ref_time = get_inputs_events.find{|x| x.is_non_stable_transition?}.timestamp
                            inputs_events_changeable.select!{|x| (x.timestamp == e.timestamp-1) or (x.timestamp == e.timestamp) or (x.timestamp == e.timestamp+1)}
                        else
                            inputs_events_changeable = get_inputs_events.select{|x| e.signal.get_source_gates.include?(x.signal)}
                            # TODO : Limiter les évènements pouvant être backtrack à ceux dans la même zone temporelle que 'event' (avant ou après le ref_time)
                            inputs_events_changeable.select!{|x| x.timestamp == (e.timestamp - e.signal.propag_time[@delay_model])}
                        end


                        # ! If other possibilities exists for some inputs, try backtracking these ones
                        # inputs_events_changeable = get_inputs_events

                        inputs_events_changeable.each do |x|
        ...
        def backtrack_previous
            backtrack(@transitions.last)
        end
    ...
    # ! : The following constraints are necessary for inertial integer delay model  
        ...
        def compute_transitions event, input_transition = nil
            ...
            if gate.instance_of? Netlist::Port and gate.is_global? and gate.is_output? 
                ...
                if gate.get_source_comp.is_a? Netlist::Constant 
                    if event.is_non_stable_transition?
                        return nil
                    else
                        if gate.get_source_comp.instance_of? Netlist::Zero 
                            if event.value != :S0
                                return nil
                            end
                        else # It is a Netlist::One
                            if event.value != :S1 
                                return nil
                            end
                        end
            ...
            if @insert_point_observable.nil?
                possible_inputs_transitions = target_path_controlling2(possible_inputs_events, event)
            end
        ...
        #         is_target_path = [@control_path.include?(gate.get_source_gates[0]), @control_path.include?(gate.get_source_gates[1])]
        ...
        def valid_controlling_events? inputs_event, gate, is_target_path
            inputs_value = [inputs_event[0].value, inputs_event[1].value]
            # if is_target_path[0] and is_target_path[1]
            ...
            if is_target_path[0] 
                if inputs_event[0].is_non_stable_transition?
                    ...
                    # ! What if it is a second pass on this gate and a non stable transition is already backpropagated on the target path ? Should we care about it ? Or is everything true/possible ?
                    # non_stable_transition_on?(gate.get_source_gates[0])
                    # ! If the target_input is delayed, and the cumulated delay is just enough to lead to a behavior modification we should let it pass
                    non_stable_transition_on?(gate.get_source_gates[0])
                else
                    non_stable_transition_on?(gate.get_source_gates[0]) or non_stable_transition_on?(gate.get_source_gates[1]) # Both inputs are stable, not any non stable transition propagated
                end
                ...
                if inputs_event[1].is_non_stable_transition?
                    ...
                    non_stable_transition_on?(gate.get_source_gates[0]) or non_stable_transition_on?(gate.get_source_gates[1])
                end
            else
                raise "Error : Unexpected situation not handled"
            end
        ...
        def target_path_controlling2 events, initial_event
            gate = initial_event.signal
            # ! Opti : Directly compute the right transitions using a method or an attribute of gate
            if gate.instance_of? Netlist::Not or gate.instance_of? Netlist::Buffer
                return events
            else
                is_target_path = [@control_path.include?(gate.get_inputs[0]), @control_path.include?(gate.get_inputs[1])]
                ...
                if (!is_target_path[0] and !is_target_path[1])
                    ...
                    if is_target_path.all?{|x| x}
                        ...
                        # # TODO : Retirer les propositions avec deux transitions non stables si gate est un xor2
                        # if gate.instance_of?(Netlist::Xor2)
                        #     events.delete_if do |proposition| 
                        #         proposition.all?{|e| e.is_non_stable_transition?}
                        #     end
                        # end

                        invert_if_not_valid_events = true
                    ...
                    valid_events = events.select do |inputs_event|
                        valid_controlling_events?(inputs_event, gate, is_target_path)
                    end

                    if valid_events.empty? and invert_if_not_valid_events
                        is_target_path[0] = !is_target_path[0]
                        is_target_path[1] = !is_target_path[1]
        ...
        def get_path_delay path, start: path[0], stop: path[-1]
            path[path.index(start)...path.index(stop)].inject(0){|sum, gate_output| sum + (gate_output.is_global? ? 0 : gate_output.partof.propag_time[@delay_model])}
        end
        ...
        def get_event_list transition_pairs, event
            ...
            if source_gate0.instance_of? Netlist::Circuit 
                raise "Error : #{gate} input i0 invalid partof"
            elsif source_gate1.instance_of? Netlist::Circuit 
                raise "Error : #{gate} input i1 invalid partof"
            end
