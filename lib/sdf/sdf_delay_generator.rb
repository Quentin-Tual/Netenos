module SDF
  class DelayGenerator < Visitor
    def initialize(netlist, function = :typ, inserted_gates: [])
      @netlist = netlist
      @delays = nil
      @fun = function
    ...
    def visit_DESIGN(subject)
      raise "Error: AST design name #{subject.data} does not match netlist name #{@netlist.name}" unless check_name(
        subject.data, @netlist.name
      )
    end
    ...
    def visit_INTERCONNECT(subject)
      ...
      if sink_name.include?(@SDF_PORT_NAME_SEP) # the source is a port of an standard cell instance
        return if sink_name.include?('ANTENNA')
    ...
    def visit_IOPATH(subject)
      raise 'Error: No current instance defined.' if @current_instance.nil?
      ...
      [source_name, sink_name, Delays::RiseFallDelay.new(*subject.delays.accept(self))]
    end

    def visit_DelayTable(subject)
      [Delays::MinTypMaxDelay.new(*subject.rise.accept(self)), Delays::MinTypMaxDelay.new(*subject.fall.accept(self))]
    end

    def visit_DelayArray(subject)
      dly = subject.send(@fun).to_f
      min_dly = (dly * 1000).to_i
      typ_dly = (dly * 1000).to_i
      max_dly = (dly * 1000).to_i
      [min_dly, typ_dly, max_dly]
    end
    ...
    def get_eq_name(name)
      ...
      raise "Instance #{name} not found in circuit #{@netlist.name}" if comp.nil?

      celltype = comp.class.name.split('::').last.downcase

      raise "Celltype #{celltype} not found in @pdk_ios (#{$PDK_IOS_JSON})" if @pdk_ios[celltype].nil?
