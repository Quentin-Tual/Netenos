  ...
  class LogDetector
    ...
    def generate_filtered_gnuplot_data(path = @log.filepath)
      ...
      @nb_outputs.times do |op_i|
        @stim_targets.each do |stim_i, (targets, _v0, _v1)|
          anomaly_distrib << if targets.any? { |t| t.last == "o#{op_i}" }
                               "#{op_i} #{stim_i} #{@log.anomaly_length((stim_i + 1) * 2, "o#{op_i}")}"
                             else
                               "#{op_i} #{stim_i} 0"
                             end
        end
    ...
    def get_suspects_index(a_dist, deg = 1) # deg is the multiplier of stddev as a distance of the mean
      ...
      distance_distrib.each do |ano_duration, stim_i|
        next unless ano_duration > a_stddev * deg
    ...
    def get_suspects_by_outputs(deg = 1)
      ...
      anomaly_len_dist_by_output.each_with_object({}) do |(o, dist), h|
