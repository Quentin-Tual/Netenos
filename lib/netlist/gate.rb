...
module Netlist
  def self.generate_gtech(max_nb_inputs = 5, excluded_gate_types = %w[Buffer Not STDCell])
    ...
    (3..max_nb_inputs).each do |nb_inputs|
      types_list.each do |gate_type|
        ...
        create_class(class_name, gate_type) unless class_exists?('Netlist::' + class_name)
      end
    end
  end
  ...
  def self.get_ruby_exp_from_pdk(class_name, pdk_fun, pdk_ios)
    raise "Error: #{class_name} not found in #{$PDK_IOS_JSON}" if pdk_ios[class_name].nil?

    if pdk_fun[class_name].nil?
      return '' if %w[tap decap fill].any? { |exception| class_name.include?(exception) }
    ...
    ruby_fun = lib_fun_ast.accept(Bexp::RubyConverter.new)
    "proc { |i| eval '#{ruby_fun}' }"
  end
  ...
  def self.get_ios_eq_tab(ios_h)
    ios_h.each_with_object({}) do |(direction, sub_h), ios_eq_tab|
      sub_h.each do |pname|
    ...
    unless Netlist.class_exists?(class_name)
      ...
      Object.class_eval <<-RUBY, __FILE__, __LINE__ + 1
        class #{class_name} < #{Netlist::STDCell}
          attr_reader :scl_ios, :scl_fun

          SMT_EXPR=#{get_SMT_exp_from_pdk(std_cell_name, pdk_fun, pdk_ios)}
          RUBY_FUN_PROC=#{get_ruby_exp_from_pdk(std_cell_name, pdk_fun, pdk_ios)}
  ...
  def self.create_gate(type, nb_inputs, partof = nil)
    ...
    create_class(specific_classname, type_classname) unless class_exists?('Netlist::' + specific_classname)
    Netlist.const_get(specific_classname).new
  end

  class Gate < Circuit
    attr_accessor :name, :ports, :partof, :propag_time, :cumulated_propag_time, :tag, :decisions,
                  :forbidden_transitions, :slack

    def initialize(name = "#{self.class.name.split('::')[1]}#{object_id}", partof = nil,
                   nb_inputs = self.class.name.split('::')[1].chars[-1].to_i, nb_outputs = 1)
      @name = name
      inputs = []
      ...
      outputs = []
      outputs << Netlist::Port.new('o0', :out) if nb_outputs > 0
    ...
    def <<(e)
      ...
      when Port
        case e.direction
        when :in
          unless @ports[:in].length < get_max_input_port
      ...
      @source_gates = nil unless @source_gates.nil?
    end
    ...
    def update_path_slack(slack)
      ...
      get_inputs.difference(crit_node.values.flatten).each do |in_p|
        ...
        raise "Error: negative slack not expected, #{in_p} has #{input_slack} slack" if input_slack < 0

        next unless in_p.slack.nil? or in_p.slack > input_slack
    ...
    def update_path_delay(elapsed, delay_model)
      ...
      return unless curr_delay > @cumulated_propag_time
      ...
      # ends the propagation if this delay is not greater than the old one
    end
  ...
  class Not < Gate
    ...
    def initialize(name = "#{self.class.name.split('::')[1]}#{object_id}", partof = nil, nb_inputs = 1,
                   nb_outputs = 1)
      super
      @propag_time = { one: 1, int: 1.0, int_multi: 2, int_rand: 1.0 * rand(0.9..1.1).round(3),
                       fract: (1.0 * rand(0.9..1.1) + 0.3).round(3) }
    end
