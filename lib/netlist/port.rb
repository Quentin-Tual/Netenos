require_relative 'wire'
module Netlist
  class Port < Wire
    attr_accessor :name, :partof, :fanin, :fanout, :direction, :slack, :decisions, :forbidden_transitions, :tag

    def initialize(name, direction, partof = nil)
      super name
      @partof = partof
      @direction = direction
      @slack = nil
      @cumulated_propag_time = 0
      # @capacitance = 0.0

      @transitions = []
      @forbidden_transitions = []
      @tag = nil
    end

    def accept(visitor)
      visitor.visit_Port(self)
    end

    def check_link_fanout?
      !@fanout.empty? and @fanout.collect { |sink| sink.get_source }.all? { |sink_source| sink_source == self }
    end

    def check_link_fanin?
      !@fanin.nil? and @fanin.get_sinks.include?(self)
    end

    def check_link?
      if @direction == :in
        if is_global?
          check_link_fanout?
        else
          check_link_fanin?
        end
      else # direction is :out
        if is_global?
          check_link_fanin?
        else
          check_link_fanout?
        end
      end
    end

    def is_free?
      if is_global? # true
        return fanout.empty? if @direction == :in

        fanin.nil?

      else
        return fanin.nil? if @direction == :in

        fanout.empty?

      end
    end

    def is_partof?
      partof.nil?
    end

    def is_input?
      @direction == :in
    end

    def is_global? # force = false
      # if @global.nil? or force
      @global = @partof.partof.nil?
      # else
      # @global
      # end
    end

    def is_output?
      @direction == :out
    end

    def is_wire?
      false
    end

    def get_full_name
      return @name if is_global?

      "#{@partof.name}#{$FULL_PORT_NAME_SEP}#{@name}"
    end

    def get_asp_name
      # return @name if is_global?
      if is_input?
        @name
      else
        "#{@partof.get_asp_name}_#{@name}"
      end
    end

    def get_dot_name
      return @name if is_global?

      "#{@partof.name}:#{@name}"
    end

    def get_pdk_name
      return "#{partof.name}#{$FULL_PORT_NAME_SEP}#{partof.scl_ios[@name]}" if !partof.nil? and partof.is_a? STDCell

      raise 'Error: no PDK name for a port in an object which is not a STDCell class object.'
    end

    def get_source_gates
      source = get_source

      if source.is_a? Netlist::Port and source.is_global?
        source
      elsif source.instance_of? Netlist::Wire
        source.fanin.partof
      else
        source.partof
      end
    end

    def get_source_comp
      if is_global? and is_input?
        self
      elsif @fanin.is_global?
        @fanin
      else
        @fanin.partof
      end
    end

    # def get_sinks_comp
    #     return get_sinks.collect{|sink|
    #         if sink.class.name == "Netlist::Wire"
    #             sink
    #         else
    #             sink.partof
    #         end
    #     }
    # end

    # def get_source_cum_propag_time
    #     return get_source.class.name == "Netlist::Wire" ? source.@cumulated_propag_time : source.partof.cumulated_propag_time
    # end

    def to_hash
      {
        port: {
          name: @name,
          partof: (@partof.nil? ? nil : @partof.name),
          direction: @direction,
          fanin: if @fanin.nil?
                   nil
                 else
                   (
                                                 @fanin.class == Wire ? @fanin.to_hash : @fanin.name
                                               )
                 end,
          fanout: if @fanout == []
                    nil
                  else
                    @fanout.collect do |interface|
                      interface.class == Wire ? interface.to_hash : interface.name
                    end
                  end
        }
      }
    end
  end
end
