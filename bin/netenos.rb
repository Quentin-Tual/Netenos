...
# ? : Use /tmp for certain use case ? Allow the persistence of the last netlist stored (more flexible and easy to use) ? Allow an interactive mode ? 
    ...
    opts.on("-g [PATH,GATE_NUMBER,INPUT_NUMBER,OUTPUT_NUMBER,STAGE_NUMBER]", "--randgen [PATH,GATE_NUMBER,INPUT_NUMBER,OUTPUT_NUMBER,STAGE_NUMBER]", Array, "Generates a random netlist according to parameters given in arguments (default: rand_circ,10,10,5,5). Stored as .enl format, may need conversion for further use.") do |randgen_args|
