...
RSpec.describe SDF::Annotator do
  ...
  context "Annotating the #{TEST_SDF_FILE} file with average values" do
    ...
    it 'applied the correct delay to gates' do
      nl = annotated_nl
      g = nl.get_component_named('_2_')
      expect(g.propag_time[:sdf]).to eq(294)
      g = nl.get_component_named('_3_')
      expect(g.propag_time[:sdf]).to eq(159)
      g = nl.get_component_named('_4_')
      expect(g.propag_time[:sdf]).to eq(159)
    end
  end
  ...
  context "Annotating the #{TEST_SDF_FILE} file with minimum values" do
    ...
    it 'applied the correct delay to gates' do
      nl = annotated_nl
      g = nl.get_component_named('_2_')
      expect(g.propag_time[:sdf]).to eq(203)
      g = nl.get_component_named('_3_')
      expect(g.propag_time[:sdf]).to eq(142)
      g = nl.get_component_named('_4_')
      expect(g.propag_time[:sdf]).to eq(142)
    end
  ...
  context 'Annotating the pnr1.v (C5315) file with typical values' do
    subject(:netlist) { Verilog.load_netlist('tests/verilog/pnr1.v') }
    subject(:annotator) { SDF::Annotator.new(netlist, :typ) }
    subject(:ast) { SDF::Parser.new.parse('tests/sdf/pnr1.sdf') }
    subject { ast.accept(annotator) }
    subject(:annotated_nl) do
      nl = netlist
      annotator_local = SDF::Annotator.new(nl, :min)
